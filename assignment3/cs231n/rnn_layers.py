...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    fh = np.einsum('ij,jk->ik', prev_h, Wh)
    fx = np.einsum('ij,jk->ik', x, Wx)
    g = fh + fx + b
    next_h = np.tanh(g)
    cache = (x, prev_h, Wx, Wh, b, fx, fh, g)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, b, fx, fh, g = cache

    cosh_g = (np.exp(g) + np.exp(-g)) / 2
    dnext_h_dg = cosh_g ** (-2)

    N, D = dnext_h.shape
    _, H = Wh.shape

    dg = dnext_h * dnext_h_dg
    db = np.sum(dg, axis=0)

    dWh = np.einsum('ij,ik->jk', prev_h, dg)

    dWx = np.einsum('ij,ik->jk', x, dg) # x.T dg

    dprev_h = np.einsum('ij,kj->ik', dg, Wh) # dg Wh.T

    dx = np.einsum('ij,kj->ik', dg, Wx) # dg Wx.T
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros(shape=[N, T, H], dtype=Wh.dtype)
    prev_h = h0
...
def rnn_backward(dh, cache):
    ...
    (N, T, H) = dh.shape
    x_T, prev_h_T, Wx, Wh, b, fx_T, fh_T, g_T = cache[T - 1]
    (_,D) = x_T.shape
    ...
    for t in range(T - 1, -1, -1):
        cache_t = cache[t]
        dx_t, dprev_h_t, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h=dnext_h, cache=cache_t)
...
def word_embedding_forward(x, W):
    ...
    one_hot = np.zeros((N, T, V), dtype=W.dtype)
    # Use advanced indexing to set the one-hot encoding
    one_hot[np.arange(N)[:, None], np.arange(T), x] = 1

    out = np.einsum('ijk,kl->ijl', one_hot, W)
    cache = (one_hot)
...
def word_embedding_backward(dout, cache):
    ...
    dW = np.einsum('ijk,ijl->kl', one_hot, dout)

...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for m in range(2 * N):
            if m != k:
                denom1 += torch.exp(sim(z_k, out[m]) / tau)

        denom2 = 0
        for m in range(2 * N):
            if m != k + N:
                denom2 += torch.exp(sim(z_k_N, out[m]) / tau)
        
        l_ij = -torch.log(num/denom1)
        l_ji = -torch.log(num/denom2)

        total_loss += l_ij + l_ji
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.einsum('nd,nd->n', out_left, out_right) / (torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1))
...
def compute_sim_matrix(out):
    ...
    inner_prod = torch.einsum('id,jd->ij', out, out)
    norms = torch.linalg.norm(out, dim=1, keepdim=True)
    norm_matrix = torch.einsum('ik,jk->ij', norms, norms)

    sim_matrix = inner_prod / norm_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = torch.exp(sim_positive_pairs(out_left, out_right) / tau)

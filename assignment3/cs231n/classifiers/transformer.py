...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        memory = memory.view([N, 1, -1])

        tgt_mask = torch.tril(torch.ones(size=[T,T])).T

        # print(f'tgt shape: {tgt.shape}, memory shape: {memory.shape}, tgt_mask shape: {tgt_mask.shape}')

        scores = self.transformer(tgt, memory, tgt_mask)

...
def affine_relu_dropout_forward(x, w, b, dropout_param):
    ...
    """
    a, fc_cache = affine_forward(x, w, b)
    relu_a, relu_cache = relu_forward(a)
    out, dropout_cache = dropout_forward(relu_a, dropout_param=dropout_param)
...
def affine_relu_dropout_backward(dout, cache):
    ...
    da = relu_backward(d_dropout, relu_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db
...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
    a, fc_cache = affine_forward(x, w, b)
    # assert a.shape[1] == gamma.shape[0]
...
def affine_layer_norm_relu_forward(x, w, b, gamma, beta, ln_param):
    a, fc_cache = affine_forward(x, w, b)
    # assert a.shape[1] == gamma.shape[0]
...
def affine_layer_norm_relu_backward(dout, cache):
